...
x_bits=randi([0 1],N_bits,1);
x_bits=x_bits';
    
x_sqaure=GenerateSquarePulses((0:N_bits-1)*Ts,T_sq,Energy_per_bit,fs,x_bits,'unipolar');
y_square = AWGNChannel(x_square,No,fs);
[rec_bits, ht, z_square] = MatchedFilter(T_sq,Energy_per_bit,fs,y_square,'unipolar');
error1=ComputeBER(x_bits,rec_bits)
...
% Hint: reuse the code from the previous cell. Your code can be as short as
% 5 lines. You can reuse the function ComputeBER from Experiment 1.
x_bits=randi(1,N_bits);
x_bits=GenerateSquarePulses((0:N_bits-1)*Ts,T_sq,Energy_per_bit,fs,1,'bipolar');
y_square = AWGNChannel(x_bits,No,fs);
[rec_bits, ht, z_square] = MatchedFilter(T_sq,Energy_per_bit,fs,y_square,'bipolar');
error2=ComputeBER(x_bits,rec_bits)
...
%%% WRITE YOUR CODE HERE
for a=1:length(Eb_No_dB_vector)
    SNR=10^((Eb_No_dB_vector(a)/10));
    No=(Energy_per_bit/SNR);
    N0(a)=No;
    x_bits=zeros(1,N_bits);
  x_bits=GenerateSquarePulses((0:N_bits-1)*Ts,T_sq,Energy_per_bit,fs,1,'unipolar');
  y_square = AWGNChannel(x_bits,No,fs);
  [rec_bits, ht, z_square] = MatchedFilter(T_sq,Energy_per_bit,fs,y_square,'unipolar');
  BER_uni(a)=ComputeBER(x_bits,rec_bits)
end
for a=1:length(Eb_No_dB_vector)
    SNR=10^((Eb_No_dB_vector(a)/10));
    No=(Energy_per_bit/SNR);
  ...
  y_square = AWGNChannel(x_bits,No,fs);
  [rec_bits, ht, z_square] = MatchedFilter(T_sq,Energy_per_bit,fs,y_square,'bipolar');
  BER_bi(a)=ComputeBER(x_bits,rec_bits)
end
