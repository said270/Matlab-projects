...
%%% WRITE YOUR CODE HERE
%% Part 4: BER for simple bit-flipping channel with multiple samples
...
% Decode bits from received bit sequence
rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_4',fs);    % IMPLEMENT THIS: Decode the received bits

% Compute the BER
BER_case_4 = ComputeBER(bit_seq,rec_bit_seq);   % Calculate the bit error rate

%% Part 4-a: Effect of repeations on BER
% GOAL: Make a plot for the BER versus different values of the repetions
% parameter fs
p=0.2;
fs_vect          = 1:1:10;              % Use this vector to extract different values of repetions in your code
...
for fs_ind = 1:length(fs_vect)
    ...
    rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_4',fs_vect(fs_ind));
